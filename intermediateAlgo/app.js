...
function orbitalPeriod(arr) {
    ...
    return arr.map(item => {
        let a = (item.avgAlt + earthRadius) ** 3;
        let amSquare = Math.sqrt(a / GM)
        return {
            name: item.name,
            orbitalPeriod: Math.round(2 * Math.PI * amSquare)
        }
    })
